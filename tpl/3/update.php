...
if (! isset($_GET["id"])) {
    echo $twig->render("3/error.html", array("pesan"=>"Error request"));
    return;
}

if (! isset($_POST["nama"]) ||
    ! isset($_POST["harga"])) {
    echo $twig->render("3/error.html", array("pesan"=>"Data produk tidak lengkap"));
    return;
}
...
if (! $rows) {
    echo $twig->render("3/error.html", array("pesan"=>"Gagal query"));
    return;
}

if ($rows->num_rows == 0) {
    echo $twig->render("3/error.html", array("pesan"=>"Produk tidak ditemukan"));
    return;
}
...
if ($image) {
    if ($image['error'] == 0 && file_exists($image['tmp_name'])) {
        ...
        echo $twig->render("3/error.html", array("pesan"=>"Masalah dengan upload file"));
        return;
    }
}
...
if ($result)
    ...
    echo $twig->render("3/error.html", array("pesan"=>"Gagal mengupdate produk"));
?>
